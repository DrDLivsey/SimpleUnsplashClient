...
protocol ImagePresenterDelegate: AnyObject {
    func presentImageItems(imageItems: [ImageItemProtocol]) -> ()
    func presentErrorMessage(errorMessage: String) -> ()
}

typealias PresenterDelegate = ImagePresenterDelegate & UIViewController

class ImagePresenter: NetworkCallerDelegate {
    
    weak var delegate: PresenterDelegate?
    private let caller = NetworkCaller()
    ...
    func prepareImageItems(currentPage: Int) -> () {
        ...
        if let cachedData = cache.getImageItems(currentPage: currentPage){
            ...
            var imageItems:[ImageItemProtocol] = []
            let caller = NetworkCaller()
            ...
            caller.setViewDelegate(delegate: self)
            caller.getData(currentPage: currentPage, urlWithPage: urlWithPage) { result in
                switch result {
    ...
    internal func getErrorCode(errorCode: Int) {
        
        var message = ""
        
        switch errorCode {
        case 200...299: message = ""
        case 300...399: message = "Redirection"
        case 400...499: message = "Server confirmed connection, access denied"
        case 500...599: message = "No connection to server"
        case 600: message = "Wrong URL"
        case 700: message = "Something went wrong"
        case 800: message = "Wrong JSON format"
        default: message = "Something defenitely wrong"
        }
        self.delegate?.presentErrorMessage(errorMessage: message)
    }
